...
void Satlike::settings() {
  ...
  if (num_vars > 321000 && num_vars < 322000 && num_clauses > 608900 &&
      num_clauses < 610000 && top_clause_weight > 6730 &&
      top_clause_weight < 6750 && max_clause_length == 23) {
...
void Satlike::allocate_memory() {
  ...
  critical_flag = new int[malloc_var_length];
...
void Satlike::build_instance_from_maxhs(Wcnf* formula) {
  istringstream iss;
  int i, v, c;
  ...
  num_clauses = formula->nSofts() + formula->nHards();
  top_clause_weight = formula->dimacsTop();
  ...
  for (; c < formula->nHards(); c++) {
    ...
    vector<Lit> lits = formula->getHard(c);
...
void Satlike::myinit(vector<int>& init_solution) {
  int v, c;
  int i, j;
  hard_unsat_nb = 0;
  soft_unsat_weight = 0;
  ...
  if (best_soln_feasible == 0) {
    ...
    for (c = 0; c < num_clauses; c++) {
      ...
      if (org_clause_weight[c] == top_clause_weight) clause_weight[c] = 0;
      else clause_weight[c] = org_clause_weight[c];
    }
  }
  ...
  for (v = 1; v <= num_vars; v++) {
    ...
    for (i = 0; i < var_lit_count[v]; ++i) {
      ...
      //encourage flipping a variable to make hard doubly satisfied
      else if (sat_count[c] == 1 && org_clause_weight[c] == top_clause_weight /*&& var_lit[v][i].sense != cur_soln[v]*/) {
        score[v] += 1; //weight of 1 is arbitrary
      }
      //discourage flipping a variable to make hard not doubly satisfied
      else if (sat_count[c] == 2 && org_clause_weight[c] == top_clause_weight && var_lit[v][i].sense == cur_soln[v]) {
        score[v] -= 1;
      }
    }
  }
  ...
  goodvar_stack_fill_pointer = 0;
  for (v = 1; v <= num_vars; v++) {
    if (/*score[v] > 0 &&*/ critical_flag[v] == 0) {
      already_in_goodvar_stack[v] = goodvar_stack_fill_pointer;
      mypush(v, goodvar_stack);
    } else {
      already_in_goodvar_stack[v] = -1;
...
void Satlike::my_update_goodvarstack1(int flipvar) {
  int v;
  // remove the vars no longer goodvar in goodvar stack
  for (int index = goodvar_stack_fill_pointer - 1; index >= 0; index--) {
    v = goodvar_stack[index];
...
void Satlike::myflip(int flipvar) {
  int i, v, c;
  int index;
  lit* clause_c;
  ...
  for (i = 0; i < var_lit_count[flipvar]; ++i) {
    ...
    if (cur_soln[flipvar] == var_lit[flipvar][i].sense) {
      ...
      if (sat_count[c] == 2)  // sat_count from 1 to 2
      {
        score[sat_var[c]] += clause_weight[c];
        if (org_clause_weight[c] == top_clause_weight) {
          critical_flag[sat_var[c]] --;
          //discourage hard from being doubly satisfied
          for (lit* p = clause_c; (v = p->var_num) != 0; p++) {
            if (p->sense == cur_soln[v]) {
              score[v] -= 1; //weight of 1 is arbitrary
            }
      ...
      if (sat_count[c] == 1)  // sat_count from 2 to 1
      {
        for (lit* p = clause_c; (v = p->var_num) != 0; p++) {
          if (p->sense == cur_soln[v]) {
            score[v] -= clause_weight[c];
            sat_var[c] = v;
            if (org_clause_weight[c] == top_clause_weight) critical_flag[v] ++;
            //break;
          }
          else if (org_clause_weight[c] == top_clause_weight /*p->sense != cur_soln[v]*/) {
            //encourage hard to be doubly satisfied
            score[v] += 1; //weight of 1 is arbitrary
          }
        }
      } else if (sat_count[c] == 0)  // sat_count from 1 to 0
  ...
  // update information of flipvar
  score[flipvar] = -org_flipvar_score;
  my_update_goodvarstack1(flipvar);
}
...
int* Satlike::continue_from_init_solution(vector<int> init_solution,
                                          Wcnf* formula,
                                          int* solver_stage) {
...
void Satlike::local_search_for_me(vector<int>& init_solution, int* solver_stage) {
  settings();
